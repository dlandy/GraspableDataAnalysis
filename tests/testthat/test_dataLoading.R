test_that("JSON Loading", {
  ...
  canvasSnippetData <- importGraspableJson(canvasSnippetFile, versions = "2.5.12", compress=F, debug=F )
  ...
  expect_equal(class(canvasSnippetData), c("gm 2.5.12"
                                           ...
                                           , "tbl"
                                           , "data.frame"))
  canvasSnippetDataShort <- importGraspableJson(canvasSnippetFile, versions = c("2.5.12"), compress=T, debug=F )
